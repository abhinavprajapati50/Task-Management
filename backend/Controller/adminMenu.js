...
exports.manageMenu = async (req, res, next) => {
  const { id, title, parent_Menu, page_slug } = req.body;
  ...
  try {
    ...
    const result_menu = await navbarMenu.create({ title, parent_Menu,  page_slug:`/${title.replace(/\s/g , "-").toLowerCase()}` }, );
    console.log("-------curMenu",  title, page_slug);
    // const update_parent_menu = await navbarMenu.update({ parent_Menu: req.body.title })
    // console.log("=======update_parent_menu",update_parent_menu);
    res
      .status(200)
      .json({
        status: true,
...
exports.updateManu = async (req, res) => {
  const { title, parent_Menu, page_slug } = req.body;
  ...
  try {
    const product = await navbarMenu.update(
      { title, parent_Menu,  page_slug:`/${title}`.replace(/\s/g , "-").toLowerCase() },
      {
        where: {
          id: req.params.id,
        },
